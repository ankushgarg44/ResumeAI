...
const MOCK_PROFILES = [
  {
    title: "Master Engineering Profile",
    profile_type: "Software Engineering",
  ...
  {
    title: "Master Product Profile",
    profile_type: "Product Management",
...
export async function POST() {
  try {
    ...
    for (const mock of MOCK_PROFILES) {
      // 1. Create base row (isProfile = true)
      const newProfile = await createResume(
        userId,
        mock.template_id,
        mock.title,
        true, // isProfile
        mock.profile_type // profileType
      );

      // 2. Save complete resume data and status
      const updated = await saveResume(newProfile.id, userId, {
        resume_data: mock.resume_data as any,
        status: mock.status,
      });

      createdProfiles.push(updated);
    }
    ...
    console.error("[POST /api/profiles/seed]", error);
    return NextResponse.json(
      { error: "Failed to seed sample profiles" },
      { status: 500 }
    );
  }
}
